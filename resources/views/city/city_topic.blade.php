                                ...
                                <div class="dropdown me-3">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenu{{ $comment['id'] }}">
                                        <li>
                                            ...
                                            href="#"
                                            data-link="{{ route('city.topic.comments', ['slug' => $topicTitleSlug]) }}">
                                                <i class="fa-solid fa-link me-2"></i> Linki Kopyala
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item text-dark"
                                            href="https://twitter.com/intent/tweet?text={{ urlencode($topicTitle . ' - ' . route('city.topic.comments', ['slug' => $topicTitleSlug])) }}"
                                            target="_blank">
                                                <i class="fa-brands fa-twitter me-2"></i> Twitter’da Paylaş
                                            </a>
                                        </li> 
                                        @if (auth()->check() && auth()->user()->id == $comment['user_id'])
                                            <li><a class="dropdown-item delete-topic text-dark" href="#" data-id="{{ $comment['id'] }}" data-type="city"><i class="fa-solid fa-trash me-3"></i>Sil</a></li>                    
                                        @endif
...
@section('js')
    ...
    <script>
        $(document).ready(function () {            
            const citySubcategories = @json($cities_topics);          
           
            $("#subcategories-list").empty();
            $("#subcategories-list-mobile").empty();
            citySubcategories.forEach(function (item) {
                const citySubCategoriesCount = item.count || 0;                    
